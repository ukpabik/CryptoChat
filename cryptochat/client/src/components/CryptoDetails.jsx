...
function CryptoDetails() {
  ...
  const crypto = cryptoData.find(
    (crypto) =>
      crypto.name === cryptoIdentifier || crypto.fullName === cryptoIdentifier,
  );
  ...
  const formatDescription = (description) => {
    ...
    return splitDesc.map((part, index) => {
      if (regex.test(part)) {
        ...
        return (
          <strong key={index}>
            {Number(Number(cleanedPart).toFixed(2)).toLocaleString()}
          </strong>
        );
      }
      return part;
        ...
        {cryptoBoxOpen ? (
          <div className="crypto-info-content">
            <div className="crypto-info-header">
              ...
              <p style={{ fontWeight: "bold" }}>
                ...
                <span
                  ...
                  className={crypto.percent_change_24h > 0 ? "green" : "red"}
                >
            ...
            <div className="crypto-desc">
              <p className="crypto-desc-text">
                <span style={{ fontWeight: "bold" }}>Description: </span>{" "}
                {formatDescription(crypto.description)}
              </p>
              <div
                className="centered"
                style={{
              ...
              >
                ...
                <a className="crypto-website" href={crypto.websites[0]}>
                  <img
                    className="crypto-logo small"
                    src={crypto.logo}
                    alt={`${crypto.name} logo`}
                  />
                </a>
